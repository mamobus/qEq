...
bool are_solutions_equal(const solution_t* solution_got, const solution_t* solution_want)
{
    ...
    if ((are_both_nan_or_equal(solution_got->x1, solution_want->x1)) && (are_both_nan_or_equal(solution_got->x2, solution_want->x2)) && (solution_got->num_of_roots == solution_want->num_of_roots)) return true;
    if ((are_both_nan_or_equal(solution_got->x1, solution_want->x2)) && (are_both_nan_or_equal(solution_got->x2, solution_want->x1)) && (solution_got->num_of_roots == solution_want->num_of_roots)) return true;
    return false; 
}
int main() 
{
    const test_data test[] = // tests
    {
    ...
    if (success_count == size_of_test) printf("\x1B[32m" "%d / %d", success_count, size_of_test);
    else printf("\x1B[31m" "%d / %d", success_count, size_of_test);
    return 0;  
}
